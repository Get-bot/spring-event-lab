...
#### 1.2.3 L3 Slice 모킹 전략: @MockitoBean 유지 + @MockkBean 확장 경로

**배경**: Spring Framework 7.0은 기존 `@MockBean`/`@SpyBean`을 **제거**하고 **BeanOverride API** 기반의 `@MockitoBean`/`@MockitoSpyBean`을 도입했다. 이는 단순한 이름 변경이 아닌, 프로토타입/팩토리 빈에 대한 예측 가능한 재정의 메커니즘을 제공한다.

**현재 결정 (v0.4)**: L3 Controller Slice는 **`@MockitoBean`(Spring 7 네이티브)** 유지.

| 접근법 | 장점 | 단점 | 적용 |
|--------|------|------|:---:|
| `@MockitoBean` (Spring 7 네이티브) | 프레임워크 기본 지원, 0 추가 코드 | Mockito DSL이 L2의 MockK DSL과 혼재 | **현재 채택** |
| Custom `@MockkBean` (BeanOverrideProcessor 구현) | 전 계층 MockK 통일, 코루틴/final class 완벽 지원 | BeanOverrideProcessor 직접 구현 필요 (초기 비용) | **향후 확장** |
| ~~springmockk 4.0.2~~ | ~~간편~~ | Spring Boot 3.0 기반, SB4 호환 X | **도입 불가** |

**Custom @MockkBean 확장 가이드 (향후)**:

Spring 7의 `BeanOverrideProcessor`를 확장하면 `@MockkBean`을 직접 구현할 수 있다:

```kotlin
// 향후 구현 시 참고용 — 현재는 @MockitoBean 사용
@Target(AnnotationTarget.FIELD)
@Retention(AnnotationRetention.RUNTIME)
@BeanOverride(MockkBeanOverrideProcessor::class)
annotation class MockkBean

class MockkBeanOverrideProcessor : BeanOverrideProcessor {
    override fun createOverride(...): BeanOverrideHandler {
        // MockK 인스턴스를 Spring 빈으로 등록
        return MockkBeanOverrideHandler(mockk(relaxed = true))
    }
}
```

이 접근이 필요해지는 시점:
- L3 테스트 수가 10개 이상으로 증가할 때
- 코루틴 기반 Controller가 도입될 때
- DSL 일관성이 팀 생산성에 영향을 줄 때

**결정**: **Kotest 6.1.0(Specs + Assertions) + MockK 1.14.9 + `@MockitoBean`(L3 한정)**. Kotest Specs를 전 계층에서 사용하되, L3의 mock DSL만 Mockito를 허용.
...
- **L3는 Spring 네이티브**: `@MockitoBean`으로 호환성 안전성 우선
...
│  EventController (@WebMvcTest + @MockitoBean EventService) │     + SpringExtension
└─────────────────────────────────────────────────────────────┘
┌─ Layer 4: Integration ─────────────────────────────────────┐
...
- [ ] `EventControllerTest` (`@WebMvcTest(EventController::class)` + `@MockitoBean EventService`)
  - Bean Validation 실패 → `INVALID_INPUT`
  - `@ParameterObject` 바인딩 (`?statuses=OPEN&statuses=READY`)
  - `@PageableDefault` 기본값 + 1-based → 0-based 변환 검증
  - 성공 응답 JSON 구조 검증
...
- **Custom @MockkBean 구현**: BeanOverrideProcessor 기반 확장은 향후 L3 테스트 증가 시 검토
...
| 라이브러리 | v0.3 | v0.4 | 변경 사유 |
|-----------|------|------|---------|
| **Kotest** | 5.9.1 | **6.1.0** | 안정판 출시. ProjectConfig 필수화, PackageConfig, Power Assert 통합, KSP 기반 테스트 발견 |
| **MockK** | 1.14.3 | **1.14.9** | 최신 안정판, Kotlin 2.3 완전 호환, 코루틴 지원 강화 |
| kotest-extensions-spring | — | **6.1.0** (신규) | Kotest Specs에서 `@SpringBootTest` / `@DataJpaTest` / `@WebMvcTest` 사용을 위한 SpringExtension |
| kotest-framework-datatest | — | **6.1.0** (신규) | `withData` 데이터 드리븐 테스팅 |
| Kover | 0.9.8 | 0.9.8 (기존) | 이미 build.gradle.kts에 포함, Plan에서 누락됐던 것을 반영 |
...
- ~~`springmockk`~~ — Spring Boot 3.0 기반, SB4 미지원
...
**`EventControllerTest`** (`@WebMvcTest` + `@MockitoBean` + Kotest FunSpec):

> Spring Boot 4 호환성을 위해 L3만 `@MockitoBean` (Spring 7 네이티브) 사용. assertions은 Kotest.
...
**참고**: Kotest `SpringExtension`이 `ProjectConfig`에 등록되어 있으므로, 생성자 주입으로 `MockMvc`와 `@MockitoBean`을 직접 받을 수 있다. `@Autowired lateinit var` 대신 **생성자 매개변수 방식**이 Kotest에서 권장된다.
...
### 4.8 Testing Tools (최종 v0.4)

| 도구 | 버전 | 용도 | 상태 |
|------|------|------|:---:|
| **Kotest runner-junit5** | **6.1.0** | **Kotest Specs 런너 (FunSpec/DescribeSpec)** | ⏳ **업그레이드** |
| **Kotest assertions-core** | **6.1.0** | **Kotlin-native assertions** | ⏳ **업그레이드** |
| **Kotest extensions-spring** | **6.1.0** | **SpringExtension (DI 지원)** | ⏳ **신규** |
| **Kotest framework-datatest** | **6.1.0** | **withData 데이터 드리븐 테스팅** | ⏳ **신규** |
| **MockK** | **1.14.9** | **Kotlin-native mock (L1~L2)** | ⏳ **업그레이드** |
| `@MockitoBean` | Spring 7 내장 | L3 Controller slice (BeanOverride API) | ✅ 기존 |
| `mockito-kotlin` | starter에 포함 | Kotlin Mockito DSL (`given`, `any()`, `argumentCaptor`) | ✅ 기존 |
| MockMvc | starter에 포함 | HTTP 레이어 | ✅ 기존 |
| Testcontainers | 2.0 | 실 DB/인프라 + `@ServiceConnection` | ✅ 기존 |
| Kover | 0.9.8 | Kotlin 전용 커버리지 측정 | ✅ 기존 (Plan 반영) |
| `kotlin-test-junit5` | Kotlin BOM | JUnit Platform 호환 | ✅ 기존 |
| Power Assert | Kotlin 2.3 내장 | 실패 시 식의 중간값 시각화 | ⏳ **신규** (컴파일러 옵션) |
...
| `@MockitoBean` | Spring 7 내장 | ✅ | Spring Framework 7.0 네이티브 API |
...
### 5.7 L3 — EventController (FunSpec + @WebMvcTest + @MockitoBean)
...
└── com/beomjin/springeventlab/
    ...
    │   ├── controller/
    │   │   └── EventControllerTest.kt     # L3 FunSpec + @WebMvcTest + @MockitoBean
...
| 12 | `EventControllerTest` (FunSpec + @MockitoBean + Kotest) | L3 | Medium |
...
- [ ] build.gradle.kts에 Kotest 6.1.0 + MockK 1.14.9 + extensions-spring + framework-datatest가 설정됐다
...
| L3 DSL 혼재 (L2 MockK vs L3 Mockito) | 일관성 저하 | Kotest assertions는 공통, mock DSL만 다름. L3 테스트 수가 작아 영향 적음. 향후 @MockkBean 확장 가능 |
...
| **0.4** | **2026-04-13** | **Kotest 6.1.0 전면 도입**: Specs(DescribeSpec/FunSpec) 전 계층 적용, JUnit 5 `@Test` 제거. `ProjectConfig`(io.kotest.provided) + `PackageConfig` 구성 체계 추가. MockK 1.14.9 업그레이드. `kotest-extensions-spring`(SpringExtension) + `kotest-framework-datatest`(withData) 신규 의존성. **Power Assert** 활성화(`-Xpower-assert`). IntegrationTestBase **@ServiceConnection** 리팩토링. 병렬 실행 전략 추가. Custom @MockkBean(BeanOverrideProcessor) 확장 경로 문서화. Kover Plan 반영. | beomjin |
