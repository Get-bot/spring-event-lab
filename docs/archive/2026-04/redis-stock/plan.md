...
|----|-------------|----------|--------|
| FR-01 | 발급 API 첫 호출 시 Redis에 재고 Lazy Init (`SET NX EX`) | High | **Done** |
| FR-02 | Lua 스크립트로 중복체크+차감+기록을 원자적 실행 + issued Set TTL 보장 | High | **Done** |
| FR-03 | 수량 소진 시 매진 응답 (Redis에서 판단, DB 조회 없음) | High | **Done** |
| FR-04 | 동일 유저 중복 발급 방지 (Lua 내 SISMEMBER + DB Unique 이중 방어) | High | **Done** |
| FR-05 | 이벤트 시간 범위 외 요청 차단 | Medium | **Done** |
| FR-06 | Redis→DB 저장 실패 시 예외 유형별 보상 분기 (UK→INCR만, 기타→SREM+INCR) | High | **Done** |
| FR-07 | Redis 장애 시 503 `REDIS_UNAVAILABLE` 응답 (GlobalExceptionHandler) | Medium | **Done** |
| FR-08 | Redis 장애 시 CircuitBreaker Fallback | Low | Pending |
...
1. [시간 검증] 현재 시각이 startedAt ~ endedAt 범위인지 확인
   └─ 범위 밖 → EVENT_NOT_OPEN (409 Conflict)

2. [재고 초기화] Redis에 stock 키가 없으면 Lazy Init
   └─ SET coupon:stock:{eventId} {totalQuantity} NX EX {ttl}

3. [Lua 스크립트 실행] 중복체크 + 수량차감 + 발급기록 원자적 실행 (ARGV[2]로 ttlSeconds 전달)
   └─ 반환값 -1 → COUPON_ALREADY_ISSUED (409 Conflict)
   └─ 반환값  0 → EVENT_SOLD_OUT (410 Gone)
   └─ 반환값  1 → 발급 성공 + issued Set에 TTL 설정 (최초 1회)

4. [DB 저장] CouponIssueTxService.saveOrCompensate() — 별도 @Transactional
   └─ saveAndFlush로 즉시 INSERT
   └─ UK 위반(DataIntegrityViolationException) → INCR만 (issued Set 유지) + COUPON_ALREADY_ISSUED
   └─ 기타 DB 오류(DataAccessException) → SREM + INCR (완전 보상) + 예외 전파
...
**예외 유형에 따라 보상 전략을 분기**한다 (`CouponIssueTxService.saveOrCompensate()`).

```
[UK 위반 — DataIntegrityViolationException]
Redis Lua는 통과했지만 DB에 이미 존재 → Redis issued Set은 정확한 상태
1. INCR coupon:stock:{eventId}             ← 재고만 복원
2. COUPON_ALREADY_ISSUED (409) 응답         ← issued Set은 유지!

[기타 DB 오류 — DataAccessException]
Redis 성공 → DB 실패 → Redis 상태를 완전 롤백
1. SREM coupon:issued:{eventId} {userId}   ← 발급 기록 제거
2. INCR coupon:stock:{eventId}             ← 재고 복원
3. 예외를 클라이언트에 전파 (재시도 유도)
```

> **왜 UK 위반을 분리하는가?**: 무조건 SREM하면 issued Set에서 userId가 빠져, 다음 요청이 Redis를 또 통과하여 무한 발급 루프에 빠진다.
> **왜 saveAndFlush인가?**: JPA는 `save()` 시점에 INSERT를 지연(deferred flush)할 수 있다. `saveAndFlush()`로 즉시 INSERT해야 catch 블록에서 예외를 잡을 수 있다.
>
...
| `coupon:issued:{$eventId}` | Set | 발급된 userId 집합 | 이벤트 종료 + 1h | Lua 스크립트 내 최초 SADD 성공 시 `EXPIRE` 설정 (TTL == -1 체크) |

> **Hash Tag `{$eventId}`**: Redis Cluster 환경에서 두 키가 같은 hash slot에 배치되도록 보장. Lua 스크립트가 2개 키를 동시에 다루므로 `CROSSSLOT` 에러 방지.
...
|------|-----------|-------------|
| 이벤트 미존재 | `EVENT_NOT_FOUND` | 404 Not Found |
| 이벤트 미오픈/종료 | `EVENT_NOT_OPEN` | 409 Conflict |
| 이미 발급됨 | `COUPON_ALREADY_ISSUED` | 409 Conflict |
| 매진 | `EVENT_SOLD_OUT` | 410 Gone |
| Redis 장애 | `REDIS_UNAVAILABLE` | 503 Service Unavailable |
| 기타 서버 오류 | `INTERNAL_SERVER_ERROR` | 500 Internal Server Error |

---

## 7. Success Criteria

- [x] Lua 스크립트가 중복체크+차감+기록을 원자적으로 실행한다
- [x] 수량이 0이 되면 이후 요청은 모두 매진 응답을 받는다
- [x] 동일 유저가 2번 요청하면 두 번째는 거부된다 (Lua SISMEMBER + DB Unique 이중 방어)
- [ ] Redis와 DB의 발급 건수가 일치한다 (concurrency-test에서 검증 예정)
- [x] DB 저장 실패 시 예외 유형별 보상 처리가 동작한다 (UK→INCR만, 기타→SREM+INCR)
- [x] Redis `coupon:issued:{eventId}` Set에 TTL이 설정된다 (Lua 내 EXPIRE)
- [x] Redis 장애 시 503 REDIS_UNAVAILABLE로 응답한다
...
| 0.3 | 2026-04-16 | 구현 반영: Lua TTL(ARGV[2]), CouponIssueTxService 분리, UK-aware 보상 분기, hash tag, REDIS_UNAVAILABLE 핸들러. FR 상태 Done 갱신 | beomjin |
